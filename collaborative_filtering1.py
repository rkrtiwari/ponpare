...
def get_performance_stat(test_users_purchase, test_users_recommendation, training_user_ids):
    ...
    for key in test_users_purchase:
        ...
        correct_recommendation = [x for x in items_recommended if x in list(items_bought)]
...
# 13. find out prediction stats when the top items are recommended
train_bought = train.loc[train.PURCHASE_FLG == 1]
top_10_train = train_bought.groupby(by = 'VIEW_COUPON_ID_hash').count().sort_values(by = 'PURCHASE_FLG', ascending = False).iloc[:10]
top_10_train_coupons = top_10_train.index.tolist()        
prediction_stat_top_items = get_performance_stat_popular_item(test_users_purchase, top_10_train_coupons, training_user_ids)
...
# plot the result
# zero match, test user in training data
...
# two match, test user in training data             
...
# three match, test user in training data
...
###############################################################################
# testing why the algorithm fails when there is only one buy item in the test data
# for a user
###############################################################################
# find out test users with one purchase
test_users_purchase_count = test.groupby(by = 'USER_ID_hash', as_index = False).sum()
ind = test_users_purchase_count.PURCHASE_FLG == 1
test_users_with_one_purchase = test_users_purchase_count.loc[ind,]

# find out all coupons as well as coupons in test and training data
coupons_in_data = coupon_visit_selected_users.VIEW_COUPON_ID_hash.unique().tolist()
coupons_in_training_data = train.VIEW_COUPON_ID_hash.unique().tolist()
coupons_in_test_data = test.VIEW_COUPON_ID_hash.unique().tolist()
all_coupons = set(coupons_in_training_data + coupons_in_test_data)
len(all_coupons)
len(coupons_in_data)

# make a dictionary of coupon_cluster_id
coupon_clust_dict = {}
for coupon in coupons_in_data:
    print coupon
    ind = coupon_list_train.COUPON_ID_hash == coupon
    genre = list(coupon_list_train.loc[ind].GENRE_NAME)
    price_cat = list(coupon_list_train.price_cat.loc[ind])
    price_rate_cat = list(coupon_list_train.loc[ind].price_rate_cat)
    value = genre + price_cat + price_rate_cat
    coupon_clust_dict[coupon] = value
# checking the entries in the dictionary
for key in coupon_clust_dict:
    print key, coupon_clust_dict[key]                      

# checkin the recommendtion one by one for the users that have only once
i = 1
user_id = test_users_with_one_purchase.USER_ID_hash.iloc[i]
ind = coupon_visit_selected_users.PURCHASE_FLG == 1
purchased_coupons = coupon_visit_selected_users.loc[ind]
ind = coupon_visit_selected_users.USER_ID_hash == user_id
items_bought_by_user = purchased_coupons.VIEW_COUPON_ID_hash.loc[ind]

for item in items_bought_by_user:
    print item, coupon_clust_dict[item][0], coupon_clust_dict[item][1], coupon_clust_dict[item][2]

user_ind = get_rating_matrix_ind_for_test_user(user_id, R)

items_recommeded_to_user = get_recommendation_for_user(R, R_full, user_ind)

for item in chain(items_bought_by_user, items_recommeded_to_user):
    print item, coupon_clust_dict[item][0], coupon_clust_dict[item][1], coupon_clust_dict[item][2]

# items bought during the test period
ind = (test.USER_ID_hash == user_id) & (test.PURCHASE_FLG == 1)
test.loc[ind]

correct_recommendation = [x for x in items_recommeded_to_user if x in items_bought_by_user]
correct_recommendation = [x for x in items_recommeded_to_user if x in list(items_bought_by_user)]
prediction_stat[user_id]
test_users_purchase[user_id]
test_users_recommendation[user_id]

for key in test_users_recommendation:
    if key == user_id:
        print key, test_users_recommendation[key]
